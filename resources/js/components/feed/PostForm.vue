<template>
	...
	>
		<div class="flex items-center">
            <img v-if="authStore.user?.profile_photo" :src="authStore.user?.profile_photo" alt="User Avatar" class="w-10 h-10 rounded-full mr-3" />
		...
		<form v-if="expanded" class="mt-4" @submit.prevent="submitPost">
            ...
            <div class="relative">
                <div ref="editor" contenteditable="true" @input="handleInput" @keydown="handleKeyDown"
                    class="w-full placeholder:text-slate-400 min-h-[100px] text-sm border rounded-md px-3 py-2 transition duration-300 ease focus:outline-none shadow-sm bg-transparent text-slate-700 border-slate-200 hover:border-slate-300 focus:border-slate-400"
